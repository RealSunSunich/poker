...
public class Main extends Application{
	...
	public void start(Stage primaryStage) throws Exception {
		...
		for(int i=0; i<=cardsIdx.length-1;i++){
			cardsIdx[i]=i;
		}
...
//		WritableImage[] cardsImg = new WritableImage[52];
		...
		for (int i = cardsIdx.length; i > 1; i--) {
            swap(cardsIdx, i - 1, random.nextInt(i));
        }
...
//		img.getPixelReader().
		...
		Scene ms = new Scene(pane,1024,768);
...
//				ServerSocket ss = serverChannel.socket();
			...
			int cardHead=0;
			drawCards(img, gc, cardsIdx[cardHead++], 0);
			drawCards(img, gc, cardsIdx[cardHead++], 0);
			primaryStage.show();
				...
				while (iterator.hasNext()) {
					...
					try {
						if (key.isAcceptable()) {
							...
							for(; cardHead<=(2*players.size()-1); cardHead++){
								int nextInt = cardsIdx[cardHead];
								System.out.println(nextInt);

								Player next = player.next();
								System.out.println(next.ip);
								System.out.println(client.getLocalAddress().toString());
								if(client.getLocalAddress().toString().equals(next.ip))
									buffer.putInt(nextInt);

								drawCards(img, gc, nextInt, next.num);

								if(cardHead%players.size()==0){
									counter++;
								}
